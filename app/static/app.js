  ...
  async function renderOnlineSidebar() {
    ...
    try {
      ...
      box.innerHTML = players.map(p => {
        ...
        return `<div class="flex items-center">${dot}${avatar}<a class="truncate no-underline hover:underline underline-offset-2" href="${profile}" target="_blank" rel="noopener">${name}</a></div>`;
      }).join('');
    } catch {
